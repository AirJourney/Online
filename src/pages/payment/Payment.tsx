...
import { displayLocaleTime, getCityNameByCityCode, useCurrencySymbol } from '@common/utils';
...
const PAYPAL_CLIENT_ID = 'ATAfzqF1cSEhA7oOOyxEC48fC1gVcvV-HE9jN7aHbcR5QP9lFNhRAgk-iLIJaBhrEhr7p2n2enLLE9Bx'
...
export function Payment() {
    ...
    const [isProcrssing, setIsProcrssing] = useState(false)
    ...
    const getOrderDetailResult = async () => {
        ...
        if (response.ok) {
            ...
            // 更新stroe
            dispatch(setHeader(update(headData, { currency: { $set: response?.data.content?.currency } })));
        } else {
            error && message.error(error.message);
        }
    }
    ...
    const paypalCreateOrder = async (data: any, actions: any) => {
        ...
        const {currencyCode, txnAmount} = res
    ...
    const allpayxCreateOrder = async () => {
        if(isProcrssing) return
        setIsProcrssing(true)
    ...
    const checkOrderStatus = async () => {
        ...
        if (res.code === 200 && res.status === 0) {
            ...
            navigate(`/payment/success?res=OrderID [${state.current?.orderId}]: Payment succeeded`)
        }
    }
    ...
    return (
        <div className="payment-page-wrappr">
            ...
            <div className="content-box">
                <div className="left-box">
                    <div className="pay-tab-box">
                    {
                        paymentList.map((item, index) => {
                            return (
                                <div className={classnames("pay-tab", { active: item.checked })}  key={index} onClick={() => { handlePayMethodChange(index) }}>
                    ...
                    {/* <div className={classnames("pay-content", { active: paymentMethod === 'Enets' })}>
                    ...
                    <div className={classnames("pay-content", { active: paymentMethod === 'PayPal' })}>
                        <PayPalScriptProvider options={paypalOptions}>
                            <PayPalButtons createOrder={paypalCreateOrder} onApprove={paypalOnApprove}></PayPalButtons>
                        </PayPalScriptProvider>
                    </div>
                    <div className={classnames("pay-content", { active: paymentMethod === 'Allpayx' })}>
                        <div className="pay-btn" onClick={allpayxCreateOrder}>
                            {t('Pay Now')}
                        </div>
                    </div>
                    <div className={classnames("pay-content", { active: paymentMethod === 'Huipay' })}>
                ...
                <div className="right-box">
                    <div className="price-box">
                        <p>{t("Total: ")}</p>
                        <p className='total'>{currency} {flightData?.policyDetailInfo.totalPrice}</p>
                    </div>
                    <Divider dashed />
                        ...
                        return (
                            <div className="flight-top-info" key={index}>
                                <div className="top-info-box">
                                    <p className="city">{flight.flightSegments[0].dCityInfo.code}</p>
                                    <img
                                        className="airline-logo"
                                        src="/static/image/home/icon-plane.png"
                                    ></img>
                                    <p className="city right">{flight.flightSegments[flight.flightSegments.length - 1].aCityInfo.code}</p>
                                </div>
                                <div className="center-info-box">
                                    <p className="time">{departMultCityName}({flight.flightSegments[0].dPortInfo.code})</p>
                                    <p className="time right">{arriveMultCityName}({flight.flightSegments[flight.flightSegments.length - 1].aPortInfo.code})</p>
                                </div>
                                <div className="center-info-box">
                                    <p className="time">{displayLocaleTime(flight.departDateTimeFormat,'LLL')}</p>

                                    <p className="time  right">{displayLocaleTime(flight.arriveDateTimeFormat,'LLL')}</p>
                                </div>
                    ...
                    <div className="contact-info">
                        <div className="contact-name">
                            <p className='title'><UserOutlined /> {orderData?.contactInfo.contactName}</p>
                            <p className='title'><PhoneOutlined /> {orderData?.contactInfo.phoneArea} {orderData?.contactInfo.mobilePhone}</p>
                            <p className='title'><MailOutlined /> {orderData?.contactInfo.email}</p>
                        </div>
                        {/* <div className="contact-name">
            ...
            <Modal className='code-modal' title="QR Code" open={isQrCodeModalOpen}  onCancel={handleQrCodeModalCancel}>
                <div className="code-modal-content">
                    <QRCode value={huipayQrCode} renderAs="canvas" />
                    <img className="go-huipay" src="/static/image/home/huipay.jpg"></img>
                </div>
            </Modal>             
        </div>
    );
