...
import { displayLocaleTime, getCityImageByCityCode, getCityNameByCityCode, useCurrencySymbol } from "@common/utils";
import { AppstoreAddOutlined, GlobalOutlined, SwapOutlined, SwapRightOutlined, TransactionOutlined } from "@ant-design/icons";
import { initialState, setSearchForm } from "@features/searchForm/searchFormSlice";
...
const Home = () => {
  ...
  const getRecommendList = async () => {
    ...
    if (response.ok) {
      const filterData = response.data.content.filter((ele: any) => ele.policyDetailInfo.avgPrice !== 'NaN' && ele.policyDetailInfo.totalPrice !== 'NaN').slice(0, 4)
  ...
  const getRecommendTitle = (item: any) => {
    ...
    const departMultCityName = getCityNameByCityCode(item.flightGroupInfoList[0].flightSegments[0].dCityInfo.code) || item.flightGroupInfoList[0].departMultCityName
    const arriveMultCityName = getCityNameByCityCode(item.flightGroupInfoList[0].flightSegments[flightGroupLength-1].aCityInfo.code) || item.flightGroupInfoList[0].arriveMultCityName
    if ( item.flightGroupInfoList.length === 1) {
      return <><b>{departMultCityName}</b><SwapRightOutlined /><b>{arriveMultCityName}</b></>
    } else {
      return <><b>{departMultCityName}</b><SwapOutlined /><b>{arriveMultCityName}</b></>
    }
  }
  ...
  const getRecommendTime = (item: any) => {
      return displayLocaleTime(item.flightGroupInfoList[0].departDateTimeFormat,EmomentFormatType.RECOMMEND)
  }
  ...
  const handleRecommendClick = (item: any) => {
    let tripSeach = item.flightGroupInfoList.map( (group: any) => {
      ...
      return   {
        ...
        departTime: displayLocaleTime(group.departDateTimeFormat,EmomentFormatType.RECOMMEND)
      }
    })
    const recommendForm = update(initialState, {
      ...
      flightType: {
        $set:  item.flightGroupInfoList.length === 1? EFlightType.OneWay : EFlightType.Round
      }
    })
  ...
  return (
    <div className="home-page-wrappr">
      ...
      <div className="home-page-wrappr__inner">
        <Title level={3} className='big'>{t("Let your travels begin with ease.")}</Title>
        <SearchForm onSearchClick={onSearchClick}></SearchForm>
        <div className="adv-content">
          <div className="adv-item">
            <GlobalOutlined />
            <div className="adv-item-title">{t("Global coverage")}</div>
            <div className="adv-item-info">{t("Routes cover more than 5000 cities around the world")}</div>
          </div>
          <div className="adv-item">
            <AppstoreAddOutlined />
            <div className="adv-item-title">{t("One stop service guarantee")}</div>
            <div className="adv-item-info">{t("After confirming the payment, you can enjoy the price and ticket guarantee")}</div>
          </div>
          <div className="adv-item">
            <TransactionOutlined />
            <div className="adv-item-title">{t("Reliable system")}</div>
            <div className="adv-item-info">{t("Safe and fast online booking and payment")}</div>
          </div>
        </div>
        {/* <Title level={3}>{t("Advantages of booking with SkyWingTrip")}</Title> */}
        ...
        <div className="home-recommend-wrapper">
          <Spin tip={t("Loading...")} spinning={loading} size="large">
            {recommendList.map((item: any, index) => {
              return (
                <Card onClick={() => { handleRecommendClick(item) }} cover={<SafeImage defaultImg={DEFAULT_IMAGE_2} src={getCityImageByCityCode(item.flightGroupInfoList[0].flightSegments[0].aCityInfo.code, 1)} className="city-image" />}  className="recommend-item" key={index}>
                  ...
                  <p className="price"><b>{currency + item.policyDetailInfo.avgPrice} </b></p>
                </Card>
              )
            })}
          </Spin>
