...
export function AddressSearch(props: {
  ...
  // 获取搜索城市
  const getSearchCity = ( code: string ) => {
    const res = searchData.find( city => city.Code === code )
    return res || code
  }
  ...
  const getAirportName = (airport: any) => {
    switch (i18n.language) {
      case 'en': return airport.EName;
      case 'tc': return airport.NameTW;
      case 'cn': return airport.Name;
      case 'jp': return airport.NameJP;
    }
  }

  // 获取城市搜索名称
  const getAirportSearch = (airport: any) => {
    return airport.EName + ' ' + airport.NameTW + ' ' + airport.Spell+ ' ' + airport.Code+ ' '+ airport.ShortSpell
  }
  ...
  const filterOption = (input: any, option: any) : boolean => {
    return option && option.search && option.search.toLowerCase().indexOf(input.toLowerCase()) >= 0
  }
  ...
  return (
    ...
    >
        {
          !isSearch && Object.keys(hotCityCode).map( (area, aindex) => {
            return <OptGroup label={t(area)} key={aindex}>
              {
          ...
          {           
          isSearch && searchData.map((region, rindex) => {
            return (
              <OptGroup label={getAirportName(region)} key={rindex}>
                {
                  region.Datas.length > 1 && !region.CountryCode && <Option
                  className="ctiy-name"
                  value={region.Code || region.CountryCode}
                  key={rindex * 100}
                  search={getAirportSearch(region)}
                  label={region.EName + " " + region.NameTW + " " + region.Code + " " + region.ShortSpell}
                  showlabel={getAirportName(region) + ' - ' + t(" All Airport")}
                >
                  {getAirportName(region)}{t(" All Airport")}
                </Option> 
                }
                {
                  region.Datas.map((airport: any, aindex: number) => {
                    ...
                    label={airport.EName + " " + airport.NameTW + " " + airport.Code}
                    search={getAirportSearch(airport) + getAirportSearch(region)}
                    showlabel={getAirportName(region) + ' - ' + getAirportName(airport) + '(' + airport.Code + ')' }
                  >
