...
export const Book = () => {
  ...
  const generateFlightInfo = async (params: any) => {
    ...
    if (response.ok) {
      setShoppingId(response.data.content.shoppingId)
  ...
  // 乘客表单填写完整后，创建订单
  useEffect(() => {
    const count = passengerList.reduce((pre, cur) => {
      return cur.count+pre
    },0)
    if (orderCreated || submitData.flightPassengerList.length !== count) {
      return;
    }
    // 设置订单已创建的标记
    if (
      ...
      submitData.flightPassengerList.length
          ...
          getFlightPrice(flightData, postBookData);
      }
  }, [submitData, submitData.flightPassengerList.length]);
