...
export const PolicyModal = (props: {
  ...
  const checkSpecialText = (text: string, type: number) => {
    return Number(text) === -1 || text === "NaN" ? type? t("policyInfo2") : t("policyInfo1") : Number(text) === 0 ? t("policyFree") :  currency + text;
  };
  ...
  const getPoliciesTable = (
    ...
    const checkData = (()=>{
      const data = policyData.baggageInfoList[0].checkedFormatted?.[typeObject.key]
            ...
            <span>
              {t("Checked Baggage")}:&nbsp;
              {
                policyData.baggageInfoList[0].checkedFormatted[typeObject.key]
                  .piece + t("(s) per person") + ", " + policyData.baggageInfoList[0].checkedFormatted[typeObject.key]
                  .weight + t("kg each piece")
              }
            </span>
          </td>
        </tr>)}
          ...
          <td>
            <span>
              {t("Carry-on Baggage")}:&nbsp;
              {
                ...
                .piece + t("(s) per person")  + ", " + policyData.baggageInfoList[0].handFormatted[typeObject.key]
                .weight +  t("kg each piece")
              }
            </span>
          </td>
